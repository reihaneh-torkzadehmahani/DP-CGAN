...
def compute_epsilon(FLAGS, steps, sigma):
...
def runTensorFlow(sigma, clippingValue, batchSize, epsilon, delta, iteration):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, gpu_options=gpu_options)) as sess:
        ...
        while (step < FLAGS.num_training_steps and should_terminate == False):
            ...
            _, G_loss_curr = sess.run([G_solver, G_loss], feed_dict={Z: Z_sample, y: y_mb, lr: curr_lr})
            ...
            if (eps > max_target_eps):
                ...
                for i in range(0, 10):
                    ...
                    samples = sess.run(G_sample, feed_dict={Z: Z_sample, y: y_sample})
                    fig = plot(samples)
                    plt.savefig((resultPath + "/Final_step_{}.png").format(str(i).zfill(3)), bbox_inches='tight')
                    plt.close(fig)
                ...
                X_test, Y_test = loadlocal_mnist(
                    images_path=baseDir + "conditional-gan-dp-ours-mnist/" + 'mnist_dataset/t10k-images.idx3-ubyte',
                    labels_path=baseDir + "conditional-gan-dp-ours-mnist/" + 'mnist_dataset/t10k-labels.idx1-ubyte')
...
for iteration in range(1, 2):
    for sigma, clipping in sigma_clipping_list:
        for batchSize in batchSizeList:
            print("iRunning TensorFlow with Sigma=%f, Clipping=%d, batchSize=%d\n" % (sigma, clipping, batchSize))
